...
generic
	...
	with function Image(Value : Decimal_Type) return String is <>;
	with function Wide_Image(Value : Decimal_Type) return Wide_String is <>;
	with function Wide_Wide_Image(Value : Decimal_Type) return Wide_Wide_String is <>;
package Containers.Decimal_Stacks with Preelaborate is

	package Base_Stack is new Containers.Stacks(Decimal_Type);
	--Please ignore this
	...
	procedure Add(Self : in out Stack; Value : in Decimal_Type) with Pre => Self.Length >= 1;
	...
	procedure Subtract(Self : in out Stack; Value : in Decimal_Type) with Pre => Self.Length >= 1;
	--Subtract the specified value from the top number of the stack, and push the result back onto the stack

	procedure Sub(Self : in out Stack; Value : in Decimal_Type) renames Subtract;
	--Subtract the specified value from the top number of the stack and push the result back onto the stack
	...
	procedure Multiply(Self : in out Stack; Value : in Decimal_Type) with Pre => Self.Length >= 1;
	--Multiply the top number on the stack with the specified value, and push the result back onto the stack

	procedure Mul(Self : in out Stack; Value : in Decimal_Type) renames Multiply;
	--Multiply the top number on the stack with the specified value, and push the result back onto the stack
	...
	procedure Divide(Self : in out Stack; Value : in Decimal_Type) with Pre => Self.Length >= 1;
	--Divide the top number on the stack by the specified value, and push the result back onto the stack

	procedure Div(Self : in out Stack; Value : in Decimal_Type) renames Divide;
	--Divide the top number on the stack by the specified value, and push the result back onto the stack

end Containers.Decimal_Stacks;
