...
package Containers.Lists.Doubly_Linked with Preelaborate is
...
--@version 1.0.0
	...
	function Previous(Self : Node_Access) return Node_Access renames Afore;
	...
	procedure Hindfree(Self : in out List);
	--Free the hindmost value
	...
	function Flip(Self : in List) return List with Pure_Function;
	--Flip the order of the list
	
	overriding procedure Clear(Self : in out List);
	--Clear the list of all values

	function Copy(Self : List) return List with Pure_Function;
	--Make a copy of the list

	overriding function Length(Self : List) return Natural with Pure_Function;
	--The length of the list, or total number of values

	overriding function Size(Self : List) return Natural with Pure_Function;
	...
	--Convert the list to an array for iteration over each value
	...
	procedure Postpend(Self : in out List; Values : in Value_Array) renames Hindbind;
	--Postpend the values to the list
...
private
	...
	overriding function Last(Self : Iterator) return Cursor with Pure_Function;

	overriding function Previous(Self : Iterator; Position : Cursor) return Cursor with Pure_Function;

	function Iterate(Self : in List) return Iterator_Interfaces.Reversible_Iterator'Class with Pure_Function;
